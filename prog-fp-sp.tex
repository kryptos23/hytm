...
\begin{algorithmic}[1]
	...
	\Part{\Read$_k(X_j)$}\quad\Comment{slow-path}{
		  ...
		  \If{$r_j.\lit{isLocked}$} \label{line:abort0}
	...
	\Part{\TryC$_k$()}\quad\Comment{slow-path}{
	 ...
	 \Part{Function: $\lit{release}(Q)$}{
  		\ForAll{$X_j \in Q$}	
 			\State $r_j.\Write(r_j+1)$ \label{line:rel1}
		\EndFor
	...
	\Statex
	 \Part{Function: $\lit{isAbortable()}$ }{
		\If{$\exists X_j \in \Rset(T_k)$: $X_j\not\in \Wset(T_k)$: $r_j$ is updated}\label{line:valid}
			\Return $\true$ \EndReturn
		\EndIf
	...
	\Part{$\textit{write}_k(X_j,v)$}{\quad\Comment{fast-path}
		\If{$r_{j}\neq 0$}  \Comment{uncached}		
			\Return $A_k$ \EndReturn
		\EndIf
		\State $\Write(v_j,v)$ \Comment{cached write} 
		\State $\Write(r_j,\textit{r}_j+2)$ \Comment{uncached write}
