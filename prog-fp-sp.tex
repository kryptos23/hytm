...
\begin{algorithmic}[1]
	...
	\Part{\Read$_k(X_j)$}\quad\Comment{slow-path}{
		  ...
		  \If{$\textit{or}_j$ $\mathrel{\&} 0000...0001$} \label{line:abort0}	
			\Return $A_k$ \EndReturn
		  \EndIf
		 
		  \If{$\lit{validate}()$} \label{line:valid}
			\Return $A_k$ \EndReturn
		  \EndIf
		  \EndIf
	...
	\Part{\TryC$_k$()}\quad\Comment{slow-path}{
		...
		\If{$\lit{validate}()$} \label{line:abort3}
		...
		\ForAll{$X_j \in \Wset(T_k)$}
	 	...
	 	\EndFor	
		\State $\lit{release}(\Wset(T_k))$   \label{line:rel}	\Comment{Simple write on Haswell, LL/SC on Power 8}	
 	...
 	\Part{Function: $\lit{acquire}(Q$)}{
   		\ForAll{$X_j \in Q$}	
			\State $r_j.\lit{setV}()$ \label{line:acq1}
	 ...
	 \Part{Function: $\lit{release}(Q)$}{
  		\ForAll{$X_j \in Q$}	
 			\State $r_j.\lit{setV}()$ \label{line:rel1}
		\EndFor
	...
	\Part{Function: $\lit{setV}()$}{ \Comment{spin lock on the latest version}
		...
		\State $\ms{val} \gets$ $r_j.\Read()$ $\mathrel{\&}1111...1110$
		\State $\ms{success} \gets r_j$.CAS($val$, $val+1$)
		\EndWhile
	}\EndPart
	\Statex
	...
	\Part{$\textit{read}_k(X_j)$}\quad\Comment{fast-path}{
		...
		\If{$\textit{or}_j$ $\mathrel{\&}0000...0001$}  \label{line:hread}
			\Return $A_k$ \EndReturn
		\EndIf
	...
	\Part{$\textit{write}_k(X_j,v)$}{\quad\Comment{fast-path}
		\State $\textit{or}_j := \Read(r_j)$ \Comment{uncached read}
		\If{$\textit{or}_j$ $\mathrel{\&}0000...0001$}  		
			\Return $A_k$ \EndReturn
		\EndIf
