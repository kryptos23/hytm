...
We remark that such multi-path approaches may be easily applied to each of the Algorithms proposed in this paper.\footnote{That said, we also think it is important to aim for implementations of TM that are as simple as possible, since complex algorithms can be more difficult to verify.
For example, Invyswell adds many code paths with complex synchronization, and in doing so improves performance significantly over prior approaches.
However, one of its fastest code paths implements an optimization called \textit{lazy subscription}, which has been shown to cause opacity violations~\cite{dice2014pitfalls}.}
However, in the search for an efficient HyTM, it is important to strike the fine balance between concurrency, hardware instrumentation and software validation cost.
