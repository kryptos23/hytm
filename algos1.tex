...
Note that the accesses of the sequence lock during the t-read may be performed directly without incurring tracking set aborts and the resulting implementation would still be opaque.
...
during the tryCommit. Fast-path transactions simply check if this lock is held without acquiring it (similar to TLE~\cite{tle}). While the per-read instrumentation overhead
