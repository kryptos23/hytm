...
seem to suggest that (i) the \emph{cost of concurrency} also exists in practice; Algorithm~1 which is truly progressive performs poorly on Intel
and even more so on IBM Power8 (ii) 
...
without using global contending bottlenecks as demonstrated by Algorithm~2 which scales well on both Intel and Power8 (iii) 
...
while \cref{sec:hytmalgos} presents several opaque HyTM algorithms that are progressive for a subset of transactions.
\cref{sec:eval} presents our results from our experiments on Intel Haswell and IBM Power8 architectures which provide a clear characterization of the cost
