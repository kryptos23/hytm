...
%
Algorithm~\ref{alg:inswrite} performs terribly on Power8: Power8 transactions can only load 64 cache lines before they will abort~\cite{nguyen-thesis}. 
...
it is incredibly important either to have little or no instrumentation in transactions, or for metadata to be located in the 
same cache lines as program data. Of course, the latter is not possible for hybrid tms, which do not have control over the layout of program data.
...
This is due to the fact that fetch-increment on a single location experiences severe negative scaling on the Power8 processes: for e.g., in one second, a single
thread can perform 37 fetch-increment operations while 6 threads perform a total of 9 million and 24 threads perform only 4 million fetch-increment operations.
...
fetch-increment instructions while it must be emulated on the Ppower8 processor (using load-linked/store-conditional).
