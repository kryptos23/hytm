...
#  define STM_BEGIN(isReadOnly) \
    do { \
        SOFTWARE_BARRIER; \
        sharedReadFunPtr = &TxLoad_fasthtm; \
        sharedWriteFunPtr = &TxStore_fasthtm; \
        SOFTWARE_BARRIER; \
        ...
        /* fast htm attempts */ \
        unsigned ___fasthtmattempts; \
        for (___fasthtmattempts = 0; ___fasthtmattempts < HTM_ATTEMPT_THRESH; ++___fasthtmattempts) { \
            if (XBEGIN(___xarg)) { \
                if (fallbackCount) XABORT(1); \
                break; \
            } else { /* if we aborted */ \
        ...
        if (___fasthtmattempts < HTM_ATTEMPT_THRESH) break; \
        /* slow (regular) htm attempts */ \
        unsigned ___slowhtmattempts; \
        for (___slowhtmattempts = 0; ___slowhtmattempts < HTM_ATTEMPT_THRESH; ++___slowhtmattempts) { \
            if (XBEGIN(___xarg)) { \
                break; \
            } else { /* if we aborted */ \
                TM_REGISTER_ABORT(PATH_SLOW_HTM, ___xarg, ___Self->UniqID); \
                ++___Self->AbortsHW; \
            } \
        } \
        /*printf("exited loop\n");*/ \
