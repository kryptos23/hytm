    ...
    public:
        ...
        T* allocate(const int tid) {
            ...
            if (!mem[tid*PREFETCH_SIZE_WORDS] || bump_memory_full(tid)) {
                ...
                DS_DEBUG {
                    this->__debug->addAllocated(tid, memBytes[tid*PREFETCH_SIZE_WORDS] / cachelines / BYTES_IN_CACHE_LINE);
                    VERBOSE DS_DEBUG2 {
//                        if ((this->__debug->getAllocated(tid) % 2000) == 0) {
//                            this->debugInterfaces->reclaim->debugPrintStatus(tid);
//                            debugPrintStatus(tid);
                            COUTATOMICTID("allocated "<<(memBytes[tid*PREFETCH_SIZE_WORDS] / cachelines / BYTES_IN_CACHE_LINE)/*this->__debug->getAllocated(tid)*/<<" records of size "<<sizeof(T)<<endl);
//                            COUTATOMIC(" ");
...
//                        }
        ...
        allocator_bump(const int numProcesses, debugInfo * const _debug)
            ...
            VERBOSE DS_DEBUG COUTATOMIC("constructor allocator_bump"<<endl);
