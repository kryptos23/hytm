...
public:
    ...
    inline bool qProtect(const int tid, T * const obj, CallbackType notRetiredCallback, CallbackArg callbackArg, bool memoryBarrier = true) {
        ...
        int __size; DS_DEBUG __size = announce[tid]->size();
        DS_DEBUG assert(__size < MAX_PROTECT_EVEN_IF_QUIESCENT);
        announce[tid]->add(obj);
        assert(announce[tid]->contains(obj));
        DS_DEBUG assert(announce[tid]->size() == __size+1);
        ...
        if (notRetiredCallback(callbackArg)) {
            ...
            DS_DEBUG assert(__size == announce[tid]->size());
            return false;
        }
    }
    ...
    reclaimer_debraplus(const int numProcesses, Pool *_pool, debugInfo * const _debug, RecoveryMgr<void *> * const _recoveryMgr = NULL)
        ...
        VERBOSE DS_DEBUG COUTATOMIC("constructor reclaimer_debraplus helping="<<this->shouldHelp()<<endl);// scanThreshold="<<scanThreshold<<endl;
    ...
    ~reclaimer_debraplus() {
        VERBOSE DS_DEBUG COUTATOMIC("destructor reclaimer_debraplus"<<endl);
