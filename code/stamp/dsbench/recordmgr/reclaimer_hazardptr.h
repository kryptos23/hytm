...
public:
    ...
    inline bool protect(const int tid, T * const obj, CallbackType notRetiredCallback, CallbackArg callbackArg, bool memoryBarrier = true) {
        ...
        int size; DS_DEBUG size = announce[tid]->size();
//        DS_DEBUG if (sizeof(T) < 80 /* is a node */) assert(!announce[tid]->contains(obj));
        ...
        DS_DEBUG assert(isProtected(tid, obj)); //announce[tid]->contains(obj));
        DS_DEBUG assert(size + 1 == announce[tid]->size());
//        SOFTWARE_BARRIER;
        if (notRetiredCallback(callbackArg)) {
//            SOFTWARE_BARRIER;
            TRACE cout<<"notRetiredCallback returns true"<<endl;
            DS_DEBUG assert(announce[tid]->size() <= MAX_HAZARDPTRS_PER_THREAD);
            DS_DEBUG assert(isProtected(tid, obj));
//            SOFTWARE_BARRIER;
            assert(isProtected(tid, obj));
            return true;
        } else {
            TRACE cout<<"notRetiredCallback returns false"<<endl;
//            unprotectObject(tid, obj); // unnecessary to unprotect if we promise to enter a quiescent state as soon as we fail to protect an object!
//            DS_DEBUG if (sizeof(T) < 80 /* is a node */) assert(!isProtected(tid, obj));
//            SOFTWARE_BARRIER;
            return false;
        }
    }
...
//        announce[tid]->clearWithoutFreeingElements();
        DS_DEBUG assert(announce[tid]->size() == 0);
        DS_DEBUG assert(announce[tid]->isEmpty());
//        SOFTWARE_BARRIER;
    }
...
//            TRACE cout<<"    afterwards, we have "<<retired[tid]->size()<<" things waiting to be retired..."<<endl;
            ...
            for (int otherTid=0; otherTid < this->NUM_PROCESSES; ++otherTid) {
                ...
                for (int ixHP=0;ixHP<sz;++ixHP) {
                    int oldSize; DS_DEBUG2 oldSize = comparing[tid]->size();
                    comparing[tid]->insert(announce[otherTid]->get(ixHP));
                    DS_DEBUG2 assert(comparing[tid]->size() <= oldSize + 1); // might not increase size if comparing[tid] already contains this item...
                }
            }
            for (int ix=0;ix<retired[tid]->size();) {
                // check if retired[tid]->data[ix] is in any set of hazard pointers
            ...
            DS_DEBUG assert(!retired[tid]->isFull());
        }
    }
    ...
    reclaimer_hazardptr(const int numProcesses, Pool *_pool, debugInfo * const _debug, RecoveryMgr<void *> * const _recoveryMgr = NULL)
        ...
        VERBOSE DS_DEBUG cout<<"constructor reclaimer_hazardptr"<<endl;
    ...
    ~reclaimer_hazardptr() {
        VERBOSE DS_DEBUG cout<<"destructor reclaimer_hazardptr"<<endl;
