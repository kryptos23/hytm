...
//                    DS_DEBUG assert(stateOld >= state_aborted);
//                    DS_DEBUG assert(stateNew >= state_aborted);
...
//            DS_DEBUG { // some debug invariant checking
...
//                DS_DEBUG if (j < highestIndexReached) {
...
//        DS_DEBUG counters->llxFail->inc(tid);
//        return NULL;
//    } // return and retry
...
//            DS_DEBUG counters->llxSuccess->inc(tid);
...
__rtm_force_inline void * abtree<DEGREE,K,Compare,RecManager>::llx_txn(
    ...
    bool marked = node->marked;
    if (marked) {
//        /** DS_DEBUG **/ XEND(); cout<<"ERROR: ENCOUNTERED MARKED NODE"<<endl; exit(-1);
        return NULL;
    } else if (state) {
...
//    /** DS_DEBUG **/ XEND(); cout<<"ERROR: ENCOUNTERED NODE WITH STATE "<<state<<" VERSION#? "<<IS_VERSION_NUMBER(scx1)<<endl; exit(-1);
    return NULL; // fail
}
