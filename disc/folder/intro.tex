...
%
%In our experiments, we considered two types of workloads: workloads in which essentially all transactions commit on the fast-path, 
%and workloads in which some thread periodically performs transactions on the slow-path.
%
Collectively, our results suggest %experiments with Intel and IBM POWER8 HTMs suggest
the following.
...
%(ii) It is important to implement HyTMs that provide progressiveness for a maximal set of transactions without incurring high hardware instrumentation overhead or using global contending bottlenecks.
%(iii) There is no easy way to derive more efficient HyTMs by taking advantage of non-speculative accesses supported within the fast-path in POWER8 processors. %(but not in Intel HTMs).
%%We explore the concurrency vs. hardware instrumentation vs. software validation trade-offs for these algorithms.
...
\cref{sec:rel} presents the related work along with concluding remarks. Formal proofs %and pseudocodes 
appear %are provided
in the Appendix.
%