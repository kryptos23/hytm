...
\begin{center}
	...
	\\
	\vspace{2mm}
	\subfloat[fig3 \label{sfig:inv-3}]{\scalebox{0.6}[0.6]{\input{dap3}}}
	\caption{proof steps
        \label{fig:indis}} 
\end{center}
\end{figure*}
%
In this section, we show that it is impossible to implement opaque proressive HyTMs with invisible reads
...
$\tau_{2}$ is invalidated. Thus transaction $T_{2}$ incurs a tracking set abort forcing
it to return $A_{2}$ in any extension of $E\cdot e_2\cdot e_1$---a contradiction
to the assumption that $\mathcal{M}$ is progressive.   
\end{enumerate}
\end{proof}
%
