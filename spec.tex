...
\begin{center}
	...
	writtten by fast-path transaction $T_i$\label{sfig:inv-1}]{\scalebox{0.65}[0.65]{\input{dap}}}
        \\
        \vspace{1mm}
	\subfloat[Fast-path transaction $T_i$ does not contend with any of the $i-1$ t-reads performed by $T_{\phi}$;
	this execution is indistinguishable to $T_{\phi}$ from \ref{sfig:inv-1} and t-read of $X_i$ must return $nv$\label{sfig:inv-2}]{\scalebox{0.65}[0.65]{\input{dap2}}}
	\\
	\vspace{1mm}
	\subfloat[To distinguish the $i-1$ different executions, t-read of $X_i$ by $T_{\phi}$ is forced
	to access $i-1$ different base objects\label{sfig:inv-3}]{\scalebox{0.65}[0.65]{\input{dap3}}}
	\caption{proof steps
        \label{fig:indis}} 
\end{center}
\end{figure*}
