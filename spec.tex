...
\begin{figure*}[t]
\begin{center}
	\subfloat[fig1\label{sfig:inv-1}]{\scalebox{0.6}[0.6]{\input{dap}}}
        \\
        \vspace{2mm}
	\subfloat[fig2 \label{sfig:inv-2}]{\scalebox{0.6}[0.6]{\input{dap2}}}
...
\label{cl:hytm}
%
For all $i\in \{2,\ldots, m\}$, $j\in \{1,2\}$ and $\ell \leq (i-1)$, if fast-path transactions $T_i$ and $T_{\ell}$ do not perform any direct nontrivial accesses of any base object in $\mathbb{E}_{1\ell}^{i}$,
then they do not contend on the same object.
\end{claim}
%
\begin{proof}
%
...
By invisible reads and Claim~\ref{cl:hytm}, the pairs of transactions $T_{\ell'}$, $T_{i}$ and $T_{\ell'}$, $T_{\ell}$
