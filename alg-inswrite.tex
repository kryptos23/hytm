\begin{algorithm}[!h]
\caption{Progressive opaque HyTM implementation that provides uninstrumented writes and invisible reads; code for process $p_i$
executing transaction $T_k$}
\label{alg:inswrite}
\begin{algorithmic}[1]
  	\begin{multicols}{2}
  	{
	...
	\Part{\Read$_k(X_j)$}\quad\Comment{slow-path}{
		\If{$X_j \not\in \Rset_k$}
		  ...
		  \If{$\exists X_j \in Rset(T_k)$:$(\textit{ov}_j,k_j)\neq \Read(v_j)$} \label{line:valid}
			\Return $A_k$ \EndReturn
		  \EndIf
% 		  \If{$\lit{isAbortable()}$} \label{line:valid}
% 		    \Return $A_k$ \EndReturn \label{line:abort0}
% 		  \EndIf
% 		  
% 		\If{$\lit{isAbortable}()$} \label{line:abort0}
% 			\Return $A_k$ \EndReturn
% 		\EndIf
		%\If{$X_j \not\in \Rset_k$}
		%\State $\textit{ov}_j := \Read(v_j)$ \label{line:read2}
		%\If{$\textit{ov}_j \neq \Read(v_j)$} \label{line:eval}
		%	\State return $A_k$
		%\EndIf
	...
	\Part{\Write$_k(X_j,v)$}\quad\Comment{slow-path}{
			...
			\State $\textit{nv}_j := v$
			\State $\Wset(T_k) := \Wset(T_k)\cup\{X_j,[\textit{ov}_j,k_j]\}$
			\Return $\ok$ \EndReturn
