...
the transaction can either \emph{commit}, in which case it appears to have executed sequentially,
...
To preclude inconsistent executions, hardware transactions in a HyTM must be \emph{instrumented} to perform additional metadata accesses to detect contention with pending software transactions.

In this paper, we identify the \emph{cost of concurrency} in HyTMs by presenting two provably \emph{opaque} HyTM algorithms that are optimal with respect to the complexity of both software and hardware transactions.
...
is progressive only for \emph{read-only} software transactions.
We show how these algorithms can support performing some subset of metadata memory accesses \emph{nonspeculatively} without violating opacity, thus, potentially reducing the number of aborts in hardware transactions.
We evaluate implementations of these algorithms on Intel Haswell, which does not support non-speculative accesses within hardware, and IBM Power8 processor, which does.
Viewed collectively, our results highlight the subtle performance tradeoffs between concurrency, instrumentation cost on hardware and complexity of software transactions in the design space of a HyTM.
\end{abstract}
