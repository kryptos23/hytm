...
%\documentclass[conference,compsocconf,letterpaper]{sig-alternate-05-2015}
...
%% conditionally set the pdf background color only on trevor's computer
\IfFileExists{trbotpc.txt}{\pagecolor{pdfbgcolor}}{}
...
(ii) it is important to build HyTMs that provide progressiveness for a maximal set of transactions, but reducing fast-path instrumentation
without using global contending bottlenecks and (iii) 
there is no easy to way to derive more efficient HyTMs by taking advantage of non-speculative accesses within the fast-path.
\end{abstract}
